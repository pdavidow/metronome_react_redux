...
test('Player component', nestOuter => {
  ...
  nestOuter.test('...Only increment loop count after loop break has finished', async(assert) => {
    ...
    embeddedAudioTest_playTicks.loopCountUpdateAfterBreak = () => {
      actual.push(store.getState().player.loopCount);
      if (iterationCount == 2) {
        embeddedAudioTest_playTicks.loopCountUpdateAfterBreak = null;
        simulate.click(getStopButton({domNode}));
